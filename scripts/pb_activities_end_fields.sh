#!/usr/bin/env bash
# activities koleksiyonuna bitiş saati: end_hour, end_minute (number)
set -euo pipefail
...
NEW='[
  {"name":"end_hour","type":"number","required":false,"presentable":true},
  {"name":"end_minute","type":"number","required":false,"presentable":true}
]'
...
if [[ "$TOADD_LEN" -eq 0 ]]; then
  echo "end_hour / end_minute zaten tanımlı."
  exit 0
fi
...
[[ "$HTTP" == "200" ]] && { echo "Tamam: end_hour, end_minute eklendi."; jq .schema "$TMP" 2>/dev/null || true; } || { echo "HTTP $HTTP" >&2; cat "$TMP"; exit 1; }
